...
@section('content')
    ...
    <div class="form-container">
        ...
        <form id="editDiemForm" class="diem-form">
            ...
            <div class="form-group">
                ...
                <input type="number" id="diem_chuyen_can" value="{{ $diem->diem_chuyen_can }}" min="0"
                    max="10" step="0.1">
            </div>

            <div class="form-group">
                <label for="diem_giua_ky">Điểm giữa kỳ:</label>
                <input type="number" id="diem_giua_ky" value="{{ $diem->diem_giua_ky }}" min="0" max="10"
                    step="0.1">
            </div>

            <div class="form-group">
                <label for="diem_cuoi_ky">Điểm cuối kỳ:</label>
                <input type="number" id="diem_cuoi_ky" value="{{ $diem->diem_cuoi_ky }}" min="0" max="10"
                    step="0.1">
            </div>
    ...
    <script>
        ...
        function updateDiem() {
            ...
            const diemChuyenCan = document.getElementById('diem_chuyen_can').value;
            const diemGiuaKy = document.getElementById('diem_giua_ky').value;
            const diemCuoiKy = document.getElementById('diem_cuoi_ky').value;

            if (diemChuyenCan !== '') diemData.diem_chuyen_can = parseFloat(diemChuyenCan);
            if (diemGiuaKy !== '') diemData.diem_giua_ky = parseFloat(diemGiuaKy);
            if (diemCuoiKy !== '') diemData.diem_cuoi_ky = parseFloat(diemCuoiKy);

            // Validate dữ liệu
            if (diemChuyenCan !== '') {
                const val = parseFloat(diemChuyenCan);
                if (!isNaN(val)) diemData.diem_chuyen_can = val;
            }

            if (diemGiuaKy !== '') {
                const val = parseFloat(diemGiuaKy);
                if (!isNaN(val)) diemData.diem_giua_ky = val;
            }

            if (diemCuoiKy !== '') {
                const val = parseFloat(diemCuoiKy);
                if (!isNaN(val)) diemData.diem_cuoi_ky = val;
            }
